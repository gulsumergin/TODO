...
function addTodoToUI(newTodo, startDate, endDate){
  ...
  todoDatesDiv.textContent = `${formattedStartDate} - ${formattedEndDate}`;
...
function addTodo(e){
  ...
  if(newTodo.trim() === "" || startDate === "" || endDate === ""){
    showAlert("danger", "Please enter a todo and select both start and end dates!");
  }
  else {
    addTodoToUI(newTodo, startDate, endDate);
    addTodoToStorage(newTodo, startDate, endDate);
  }
